...
__all__ = ['TurboConformerEncoder']

def to_int_tuple(s: str):
        return tuple(map(int, s.split(",")))

class TurboConformerEncoder(NeuralModule, StreamingEncoder, Exportable, AccessMixin):
    """
    ...
    def __init__(
        ...
        feat_out=-1,
        num_encoder_layers="4,4,8",
        subsampling_factors="4,2,2",
        subsamplings="dw_striding,dw_striding,dw_striding",
        subsampling_conv_channels="256,128,128",
        num_attention_heads="4,4,4",
        ff_expansion_factors="4,4,4",
        conv_kernel_sizes="15,15,15",
        ...
        self.encoder_dims = to_int_tuple(encoder_dims)
        self.num_encoder_layers = to_int_tuple(num_encoder_layers)
        self.subsampling_factors = to_int_tuple(subsampling_factors)
        self.subsamplings = tuple(subsamplings.split(","))
        self.subsampling_conv_channels = to_int_tuple(subsampling_conv_channels)
        self.num_attention_heads = to_int_tuple(num_attention_heads)
        self.ff_expansion_factors = to_int_tuple(ff_expansion_factors)
        self.conv_kernel_sizes = to_int_tuple(conv_kernel_sizes)
        ...
        self._feat_out = self.encoder_dims[-1]
        ...
        for i in range(num_encoders):
            if i != 0:
                feat_in = self.encoder_dims[i-1]
            encoder = ConformerEncoderBlock(
                feat_in,
                self.num_encoder_layers[i],
                self.encoder_dims[i],
                subsampling=self.subsamplings[i],
                subsampling_factor=self.subsampling_factors[i],
                subsampling_conv_channels=self.subsampling_conv_channels[i],
                ff_expansion_factor=self.ff_expansion_factors[i],
                n_heads=self.num_attention_heads[i],
                conv_kernel_size=self.conv_kernel_sizes[i],
        ...
        if feat_out > 0 and feat_out != self._feat_out:
            ...
            self.out_proj = None
            self._feat_out = encoder_dims[-1]
        self.max_audio_length = self.pos_emb_max_len
    ...
    def forward(
        ...
        for i, encoder in enumerate(self.encoders):
            ...
            # torch.cuda.nvtx.range_push(f"conf_b-{i}")
            # audio_signal, length = encoder(audio_signal=audio_signal, length=length)
            # torch.cuda.nvtx.range_pop()

            with record_function(f"conformer_block-{i}"):
                audio_signal, length = encoder(audio_signal=audio_signal, length=length)
...
class ConformerEncoderBlock(NeuralModule, StreamingEncoder, Exportable, AccessMixin):
    """
    The encoder for ASR model of Conformer.
    Based on this paper:
    'Conformer: Convolution-augmented Transformer for Speech Recognition' by Anmol Gulati et al.
    ...
    def forward(
        ...
        if isinstance(self.pre_encode, nn.Linear):
            ...
            # torch.cuda.nvtx.range_push(f"downsampling")
        ...
        for lth, (drop_prob, layer) in enumerate(zip(self.layer_drop_probs, self.layers)):
            ...
            with record_function(f"conf-l-{lth}"):
            ...
            # torch.cuda.nvtx.range_push(f"conf-l-{lth}")
            # original_signal = audio_signal
            # cache_last_channel_cur = None
            # cache_last_time_cur = None
            # audio_signal = layer(
