...
class ContextState:
    ...
    def __init__(
        ...
        self.fail = None
        self.output = None
        self.current_word_len = current_word_len
            ...
            for i, token in enumerate(tokens):
                if token not in node.next:
                    ...
                    node.next[token] = ContextState(
                        id=self.num_nodes,
                        token=token,
                        token_score=new_token_score,
                        node_score=node.node_score + new_token_score,
                        local_node_score=0
                        if is_end
                        else (node.local_node_score + new_token_score),
                        is_end=is_end,
                        current_word_len=node.current_word_len + 1
                    )
