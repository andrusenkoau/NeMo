...
class BeamRNNTInfer(Typing):
    ...
    def __init__(
        ...
        if self.beam_size == 1:
            ...
            self.search_algorithm = self.modified_adaptive_expansion_search_batch
        else:
    ...
    def modified_adaptive_expansion_search_batch(
        self, h: torch.Tensor, encoded_lengths: torch.Tensor, partial_hypotheses: Optional[Hypothesis] = None
    ) -> List[Hypothesis]:
        """
        Based on/modified from https://ieeexplore.ieee.org/document/9250505 with batched implementation

        Args:
            h: Encoded speech features (B, T_max, D_enc)

        Returns:
            nbest_hyps: (B, N-best decoding results)
        """
