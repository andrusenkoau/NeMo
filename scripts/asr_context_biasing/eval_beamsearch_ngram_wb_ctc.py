...
def beam_search_eval(
    ...
    for batch_idx in it:
        ...
        with torch.no_grad():
            ...
            # logging.warning(f"beams_batch is: {beams_batch}")
...
################################
    ...
    for batch_idx, probs in enumerate(all_probs):
        ...
        # logging.warning("--------------------")
        # logging.warning(f"preds is :{preds}")
        # raise KeyError
        ...
        if cfg.apply_context_biasing and cfg.decoding_mode == "greedy" and  wb_results[audio_file_paths[batch_idx]]:
            ...
            boosted_text = merge_alignment_with_wb_hyps(
                preds.numpy(),
                asr_model,
                wb_results[audio_file_paths[batch_idx]]
            )
        ...
        # logging.warning("******"*10)
        # logging.warning(f"Predicted text: {pred_text.text}")
        # raise KeyError
    ...
    if cfg.decoding_mode in ["beamsearch_ngram", "beamsearch"]:
        ...
        if cfg.hotwords_file and cfg.apply_context_biasing:
            ...
            # logging.warning(f"Using hotwords for context biasing: {hotwords_list}")
            # raise KeyError("Context biasing is not supported yet in this script.")
